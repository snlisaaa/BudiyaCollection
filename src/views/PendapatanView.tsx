...
import { TrendingUp, Plus, Edit2, Trash2, Search, Filter } from 'lucide-react';
...
export const PendapatanView: React.FC<PendapatanViewProps> = ({
  onOpenTransaction,
  onEditTransaction,
  onDeleteTransaction
}) => {
  ...
  const paymentBreakdown = incomeTxs.reduce(
    (acc: { [key: string]: { amount: number; count: number } }, tx) => {
      const pm = tx.paymentMethod || 'Cash';
      if (!acc[pm]) acc[pm] = { amount: 0, count: 0 };
      acc[pm].amount += tx.amount;
      acc[pm].count += 1;
      return acc;
    },
    {}
  ...
  const categories = ['All', ...Array.from(new Set(incomeTxs.map(tx => tx.category)))];
  const paymentMethods = ['All', 'Cash', 'Transfer', 'E-Wallet'];

  // Apply filters
  const filteredTxs = incomeTxs.filter((tx) => {
  ...
  return (
    <div className="space-y-6 animate-slide-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-[#1a1a1a] rounded-xl border border-brand-sand dark:border-brand-charcoal p-5 shadow-xs flex items-center justify-between">
          <div className="space-y-1">
            ...
            <p className="text-[10px] text-gray-400 dark:text-gray-500">
              Total penjualan kotor terkumpul
            </p>
          </div>
        ...
        {/* Payment breakdowns cards */}
        {['Cash', 'Transfer', 'E-Wallet'].map((pm) => {
          const stats = paymentBreakdown[pm] || { amount: 0, count: 0 };
          return (
            <div 
              key={pm}
      ...
      <div className="bg-white dark:bg-[#1a1a1a] rounded-xl border border-brand-sand dark:border-brand-charcoal p-5 shadow-xs">
        ...
        {filteredTxs.length > 0 ? (
            ...
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="border-b border-brand-sand dark:border-brand-charcoal text-gray-400 uppercase tracking-widest font-bold">
                  ...
                  <th className="pb-3 font-semibold">Deskripsi Penjualan</th>
              ...
              <tbody className="divide-y divide-brand-sand/40 dark:divide-brand-charcoal/40 text-gray-700 dark:text-gray-300">
                {filteredTxs.map((tx) => (
                  <tr key={tx.id} className="hover:bg-brand-cream/30 dark:hover:bg-brand-charcoal/20">
                    ...
                    <td className="py-3.5 font-bold text-right text-emerald-600 dark:text-emerald-400">
          ...
          <div className="flex flex-col items-center justify-center text-center text-gray-400 py-12">
            <TrendingUp className="w-12 h-12 text-gray-300 stroke-1 mb-2" />
            <p className="text-xs">Tidak ada data pendapatan yang cocok dengan kriteria filter.</p>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white dark:bg-[#1a1a1a] rounded-xl border border-brand-sand dark:border-brand-charcoal p-6 max-w-sm w-full z-10 animate-slide-in">
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500 mb-4">
              Apakah Anda yakin ingin menghapus data pendapatan ini? Data saldo dan ringkasan bulanan akan langsung berkurang.
            </p>
